...
example : python src/build_model_for_hyperparameter_search.py \ 
-p /home/wmbio/WORK/gitworking/deeptrio/data/Train_set/human_ppi_pair.tsv \
-d /home/wmbio/WORK/gitworking/deeptrio/data/Train_set/human_ppi_seq.tsv 
-e 50
-b 32 
...
# function
def main(em_dim=15, sp_drop=0.005, kernel_rate_1=0.16, strides_rate_1=0.15, kernel_rate_2=0.14, strides_rate_2=0.25, 
    ...
    model.compile(loss='categorical_crossentropy', optimizer=adam, 
                  metrics=[tfa.metrics.F1Score(num_classes=3, average="micro", threshold=0.9)])
    record_min = 0
    ...
    history_model = model.fit([x_1[0],x_2[0]], y_train[0], 
                              batch_size=batch_size, 
                              epochs=epoch_number, 
                              shuffle=True,    
                              callbacks=callbacks,
                              validation_data=([v_1[0],v_2[0]], v_y[0]))
    
    # record_min = history_model.history['val_accuracy'][0]
    record_min = history_model.history['val_f1_score'][0]
    # model.save(sub_dir_name + '/DeepTrio_search_' + str(yy) + '.h5') ##########

    del history_model
    del model

    # # model fit and tranning   
    # for n in range(epoch_number):
    #     history_model = model.fit([x_1[0],x_2[0]], y_train[0], batch_size=32, epochs=1, shuffle=True, validation_data=([v_1[0],v_2[0]], v_y[0]))
...
if __name__ == "__main__": 
    ...
    sub_dir_name = "save_model/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
